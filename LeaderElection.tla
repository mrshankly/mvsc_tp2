...
(* problem, based on the improved version of the Dolev-Klawe-Rodeh            *)
(* algorithm. With a given number of processes, this algorithms specifies     *)
(* an efficient way to collectively find and establish which is the           *)
(* highest value containedwithin a process. We apply this to our problem      *)
(* by approaching the processe's values as their id's, so we would be         *)
(* electing the process with the highest id as the leader.                    *)
(*                                                                            *)
(* The processes are arranged in a unidirectional circle, so each one         *)
(* only communicates with it's right neighbour. This communication is         *)
(* done by the exchange of two kinds of messages, M1 and M2, in phases.       *)
(* Each process can be in one of three states: active, waiting or passive.    *)
(* Active processes can only receive M1 messages, while waiting and           *)
(* passive processes can receive both. Ideally, at the end of each phase      *)
(* the processes progressively converge on the same id until an active        *)
(* process receives a M1 type message with it's id attached, signaling        *)
(* that the algorithm has succesfuly reached it's end.                        *)
(******************************************************************************)
